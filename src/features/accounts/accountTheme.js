...
export const getAccountFinancialType = (account) =>
  FINANCIAL_TYPE[typeKey(account?.type)] ?? "other_asset";
export const isAvailableCashAccount = (account) =>
  getAccountFinancialType(account) !== "liability";
...
export function cardPalette(account) {
  ...
  if (key === "credit_card") {
    return (
      CREDIT_PALETTES[override] ??
      CREDIT_PALETTES[
        CREDIT_ORDER[hashString(account?.id) % CREDIT_ORDER.length]
      ]
    );
  }
  return (
    DEBIT_PALETTES[override] ??
    DEBIT_PALETTES[DEBIT_ORDER[hashString(account?.id) % DEBIT_ORDER.length]]
  );
}
...
export const surfaceTheme = (account) =>
  SURFACE_THEME[typeKey(account?.type)] ?? SURFACE_THEME.other;
